...
static void MF_ADC_Init(void)
{
    ...
    GPIO_InitStruct.remapPin = FL_DISABLE;                                              /*配置GPIO数字重定向功能*/
    GPIO_InitStruct.analogSwitch = FL_DISABLE;                                          /*配置GPIO模拟开关功能*/
    (void)FL_GPIO_Init(GPIOE, &GPIO_InitStruct);                                        /*GPIO初始化*/
    ...
    FL_ADC_EnableSequencerChannel(ADC, FL_ADC_EXTERNAL_CH18);                            /*通道选择*/
...
//检测VDD1
//PD10 IN18
uint32_t get_VDD1_dianya()
{
	uint32_t test_shuju = 0;
	test_shuju = GetSingleChannelVoltage_POLL(FL_ADC_EXTERNAL_CH18);
	test_shuju = test_shuju*11;
	return test_shuju;
}
//检测VDD2
//PE9 IN19
uint32_t get_VDD2_dianya()
{
	uint32_t test_shuju = 0;
	test_shuju = GetSingleChannelVoltage_POLL(FL_ADC_EXTERNAL_CH19);
	test_shuju = test_shuju*11;
	return test_shuju;
}
//检测VDD3
//PA13 IN4
uint32_t get_VDD3_dianya()
{
	uint32_t test_shuju = 0;
	test_shuju = GetSingleChannelVoltage_POLL(FL_ADC_EXTERNAL_CH4);
	test_shuju = test_shuju*11;
	return test_shuju;
}
//检测VDD4
//PA14 IN11
uint32_t get_VDD4_dianya()
{
	uint32_t test_shuju = 0;
	test_shuju = GetSingleChannelVoltage_POLL(FL_ADC_EXTERNAL_CH11);
	test_shuju = test_shuju*11;
	return test_shuju;
}
//检测VDD5
//PA0 IN5
uint32_t get_VDD5_dianya()
{
	uint32_t test_shuju = 0;
	test_shuju = GetSingleChannelVoltage_POLL(FL_ADC_EXTERNAL_CH5);
	test_shuju = test_shuju*11;
	return test_shuju;
}
//检测VDD6
//PA1 IN12
uint32_t get_VDD6_dianya()
{
	uint32_t test_shuju = 0;
	test_shuju = GetSingleChannelVoltage_POLL(FL_ADC_EXTERNAL_CH12);
	test_shuju = test_shuju*11;
	return test_shuju;
}
